...
- `generator.rs` depends only on `block_type.rs`.
...
### 4.5 UI Components (internal to ui.rs)
...
main()
  ...
  +-- run() -> Result<()>
      ...
      16. GeneratorConfig { ..., allow_actions, deny_actions }
      17. generate_files(config)
            ...
            c. generate_integration_tests(config)
                ...
                -> Write providers.tf (pinned versions)
                -> Write main.tf (block stub)
...
3. **Flat module structure**: All 7 source files are at the same level (`src/*.rs`). There are no subdirectory modules. This is appropriate for the codebase size (~2,660 lines total) and the limited number of responsibilities.
...
8. **Pinned provider versions**: The generated `providers.tf` uses hardcoded version strings (`aws = "6.7.0"`, `time = "0.13.1"`, `random = "3.7.2"`). These are intentionally pinned because Renovate automation handles version updates in the mappings repository.
...
| File | Lines | Purpose |
|------|-------|---------|
| `src/main.rs` | ~197 | Entry point. Module declarations. `run()` orchestrates the full wizard pipeline. `init_logging()` configures env_logger. `validate_working_directory()` resolves and canonicalizes the path. Unit tests for path validation. |
...
| `src/generator.rs` | ~700 | `GeneratorConfig` struct with separate `allow_actions` and `deny_actions`. `generate_files()` orchestrates mapping file and test stub creation. `generate_mapping_yaml()` outputs separate `deny:` and `allow:` YAML sections (deny before allow, omitting empty sections). `generate_integration_tests()` creates directory structure with four files. URL generation helpers. `print_success_message()` outputs tree-formatted success output. `is_valid_terraform_type()` path traversal guard. `TestFiles` internal struct. Extensive unit tests including deny-section-specific scenarios. |
...
- **Non-hardcoded provider versions**: The pinned versions in `providers.tf` could become configurable or read from a config file. Currently they rely on Renovate for updates.
